...
function exportFilms(csv, mail) {
    const data = {
        sender: 'cinemax@cinemax.fr',
        receiver: mail,
        object: 'Export des films',
        content: 'Ci joint un export des films en CSV',
        attachments: [
            {
              filename: "films.csv",
              content: csv,
...
function terminateAccount(mail) {
    const data = {
        sender: 'cinemax@cinemax.fr',
        receiver: mail,
        object: 'Tu as résilié ton compte :(',
        content: 'Au revoir'
    }
    sendToMailServer(data);
}
...
module.exports = {
    welcome, newFilm, accountModified, ban, patchFilm, deleteFilm, exportFilms, terminateAccount
}